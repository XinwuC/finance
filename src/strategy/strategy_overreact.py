...
class OverReactStrategy(Strategy):
    ...
    def analysis(self, market: str, symbol: str, price_history: pandas.DataFrame,
        ...
        if top_drops.size / drop_history.size > self.top_drop_pct:
            return None
        ...
        for date in top_drops.index.tolist():
            buy_price = price_history["Adj Close"][date]
            ...
            index = price_history.index.get_loc(date)
            for day in range(1, self.recover_days):
                if index - day >= 0:
        ...
        if hit_target_price_count / top_drops.size > self.recover_success_rate \
                and hit_max_fallback_count / top_drops.size < self.allowed_max_fallback_rate:
            ...
            return pandas.Series({'date': target_date, 'market': market, 'symbol': symbol,
                                  ...
                                  'top_drop_count': top_drops.size,
                                  'drop_count': drop_history.size,
                                  'top_drop_ratio': top_drops.size / drop_history.size,
                                  'hit_targets': hit_target_price_count,
                                  'hit_target_ratio': hit_target_price_count / top_drops.size,
                                  'hit_max_fallback': hit_max_fallback_count,
                                  'max_fallback_ratio': hit_max_fallback_count / top_drops.size
