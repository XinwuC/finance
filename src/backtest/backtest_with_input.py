...
def handle_data(context, data):
    ...
    today = str(context.trading_calendar.next_open(context.datetime).replace(tzinfo=None).date())
    ...
    if len(stock_list) > 0 and not BackTestUtility.is_ramp_down(context) and \
        ...
        # new buyings
        buyings = context.input[today:today]
        for stock in buyings.itertuples():
            asset = context.screen_assets[stock.symbol]
            if data.can_trade(asset):
                BackTestUtility.cleanup_open_orders(asset)
                order_target_value(asset, context.fixed_buying_amount, style=LimitOrder(data.current(asset, 'close')))
...
def place_sell_orders(context, data, position):
    ...
    if new_sell_price > current_sell_price and new_sell_price > position.cost_basis:
